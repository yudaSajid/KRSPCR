...
import { EnrollmentRecord, PaginationMeta, AdvancedFilterPayload, CreateBatchKrsForm, CreateKrsForm } from './types';
import {
...
export const App: React.FC = () => {
  ...
  const handleCreate = async (formData: CreateBatchKrsForm | CreateKrsForm) => {
    try {
      const res = await api.createEnrollment(formData);
      showToast('success', res.message || 'KRS berhasil ditambahkan ke 3 entitas secara atomik!');
      loadData();
    } catch (err: any) {
    ...
    <div className="min-h-screen flex flex-col bg-slate-50 text-slate-800">
      ...
      {toast && (
        <div className="fixed top-20 right-6 z-50 animate-in slide-in-from-top-4 duration-300">
          <div
            className={`flex items-center gap-3 px-4 py-3 rounded-2xl shadow-xl border text-xs sm:text-sm font-semibold ${toast.type === 'success'
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-4 border-b border-slate-100">
            <div>
              <div className="flex items-center gap-2.5">
                <h2 className="text-lg sm:text-xl font-extrabold text-slate-900 tracking-tight">
                  Daftar Pengambilan Mata Kuliah (KRS)
                </h2>
            ...
            <div className="flex flex-wrap items-center gap-2.5">
              ...
              <button
                ...
                className={`flex items-center gap-2 px-4 py-2.5 rounded-2xl text-xs font-bold border transition-all ${advancedCount > 0
              ...
              <button
                ...
                className="flex items-center gap-2 px-5 py-2.5 rounded-2xl text-xs font-bold text-white bg-indigo-600 hover:bg-indigo-700 active:bg-indigo-800 shadow-md shadow-indigo-600/20 transition-all hover:scale-[1.02] active:scale-[0.98]"
              >
            ...
            <div className="flex flex-col md:flex-row items-center justify-between gap-3.5">
              ...
              <div className="relative w-full md:w-80 lg:w-96">
                ...
                <input
                  ...
                  className="w-full text-xs pl-10 pr-10 py-2.5 rounded-2xl border border-slate-300 bg-slate-50/70 focus:bg-white focus:outline-none focus:ring-2 focus:ring-indigo-100 focus:border-indigo-600 transition-all"
                />
                {searchInput && (
                  <button
              ...
              <div className="flex flex-wrap items-center gap-2.5 w-full md:w-auto justify-start md:justify-end">
                ...
                <div className="flex items-center gap-1.5 text-xs text-slate-600 bg-slate-50 px-3 py-2 rounded-2xl border border-slate-200/80">
                ...
                <button
                  onClick={() => loadData()}
                  title="Segarkan Data"
                  className="p-2 text-slate-500 hover:text-sky-600 hover:bg-slate-50 rounded-2xl border border-slate-200 transition-colors shadow-2xs"
                >
              ...
              <div className="pt-2 border-t border-slate-100 flex flex-wrap items-center gap-2 text-xs">
                ...
                {quickStatus !== 'ALL' && (
                  <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-xl bg-sky-50 text-sky-800 border border-sky-200 font-medium">
                    ...
                    <button onClick={() => setQuickStatus('ALL')} className="hover:text-sky-950">
                      <X className="w-3 h-3" />
                    </button>
                  </span>
                )}
